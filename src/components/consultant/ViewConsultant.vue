...
export default {
  ...
  methods: {
    ...
    async getConsultantEvents() {
      try {
        const response = await axios.get (`https://api-2np.herokuapp.com/api/EventsForConsultant/${this.$route.params.id}`);
    ...
    async addConsultantToEvent() {
      try {
        await axios.post("https://api-2np.herokuapp.com/api/EventsForConsultant", {
