...
export default {
  ...
  methods: {
    ...
    async setSelectedRow(event, index) {
      ...
      try {
        const response = await axios.get(`https://api-2np.herokuapp.com/api/EventCustomers/${this.currentEvent.event_id}`);
    ...
    async addCustomerToEvent () {
      try {
        await axios.post("https://api-2np.herokuapp.com/api/Registration", 
        {
      ...
      try {
        await axios.post("https://api-2np.herokuapp.com/api/DefaultRegistrationPayment");
